...
def tune_gcn(X_outer, y_outer, feature_names, graph_type, struct_grid):
    ...
    for s_params in struct_combos:
        for m_params in model_combos:
            ...
            # REMOVED: history appending logic
    ...
    return best_struct, best_model  # REMOVED: tuning_history
...
# Import the visualization module (Make sure you created visualization.py!)
...
def run_project():
    ...
    # NEW: Storage for ROC/PR Curves
    plotting_data = {
        'PSN_GCN': {'y_true': [], 'y_probs': []},
        'PSN_N2V': {'y_true': [], 'y_probs': []},
        'BIPARTITE_GCN': {'y_true': [], 'y_probs': []},
        'BIPARTITE_N2V': {'y_true': [], 'y_probs': []}
    }

    # NEW: Storage for Heatmap (we only need one example, e.g., from the first fold)
    heatmap_data = None
    ...
    for fold_idx, (train_val_idx, test_idx) in enumerate(outer_cv.split(X, y)):
        ...
        # FIX: Capture the tuning_history (3rd return value)
        ...
        with torch.no_grad():
            ...
            # FIX: Use 0.35 Threshold (or whatever you decide)
            preds = (probs >= 0.35).astype(int)
            # Pass the threshold value explicitly. The function will generate preds internally.
            metrics_storage['PSN_GCN'].append(calculate_metrics(y_te, probs, threshold=0.35))  # Pass preds directly if calc_metrics expects it, or update calc_metrics to take threshold

            # Store for Plotting
            plotting_data['PSN_GCN']['y_true'].extend(y_te)
            plotting_data['PSN_GCN']['y_probs'].extend(probs)
        ...
        with torch.no_grad():
            ...
            probs = torch.exp(logits)[:, 1].numpy()
            preds = (probs >= 0.35).astype(int)
            metrics_storage['BIPARTITE_GCN'].append(calculate_metrics(y_te, probs, threshold=0.35))

            # Store for Plotting
            plotting_data['BIPARTITE_GCN']['y_true'].extend(y_te)
            plotting_data['BIPARTITE_GCN']['y_probs'].extend(probs)
        ...
        preds = (probs >= 0.35).astype(int)
        metrics_storage['PSN_N2V'].append(calculate_metrics(y_te, probs,  threshold=0.35))

        # Store for Plotting
        plotting_data['PSN_N2V']['y_true'].extend(y_te)
        plotting_data['PSN_N2V']['y_probs'].extend(probs)
        ...
        preds = (probs >= 0.35).astype(int)
        metrics_storage['BIPARTITE_N2V'].append(calculate_metrics(y_te, probs,  threshold=0.35))

        # Store for Plotting
        plotting_data['BIPARTITE_N2V']['y_true'].extend(y_te)
        plotting_data['BIPARTITE_N2V']['y_probs'].extend(probs)
    ...
    for model_name, metric_list in metrics_storage.items():
        ...
        avg = df.mean()
        # Note: If calculate_metrics returns a dict with 'threshold', drop it before mean(), or access specific columns
        print(
    ...
    # --- GENERATE PLOTS ---
    print("\nGenerating Plots...")
    plot_model_comparison(plotting_data)
